...
describe('Cloudflare Analytics Worker', () => {
  it('blocks CORS preflight when no ALLOWED_ORIGIN set', async () => {
    ...
    const res = await workerMod.fetch(
      req('https://worker.example/ingest', { method: 'OPTIONS', headers: { Origin: 'https://app.example.com' } }),
      env as any
    );
  ...
  it('rejects unauthorized POST when token required', async () => {
    ...
    const res = await workerMod.fetch(
      req('https://worker.example/ingest', {
        ...
        body: JSON.stringify({ path: '/', referrer: '🔗 https://google.com', browser: '🧭 Chrome' }),
      }),
      env as any
    );
  ...
  it('accepts header bearer auth and posts to Slack', async () => {
    ...
    const res = await workerMod.fetch(
      req('https://worker.example/ingest', {
        ...
        body: JSON.stringify({ path: '/scores', referrer: '🔗 https://google.com', browser: '🧭 Chrome', url: 'https://site/scores' }),
      }),
      env as any
    );
  ...
  it('accepts body authToken (sendBeacon-compatible)', async () => {
    ...
    const res = await workerMod.fetch(
      req('https://worker.example/ingest', {
        method: 'POST',
        headers: jsonHeaders('https://app.example.com'),
        body: JSON.stringify({ path: '/', referrer: '🔗 direct', browser: '🧭 Safari', authToken: 'sek' }),
      }),
      env as any
    );
    expect(res.status).toBe(204);
