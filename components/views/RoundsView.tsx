...
export default function RoundsView() {
  ...
  function PlayerCell({ roundNum, tricks, playerId }: { roundNum: number; tricks: number; playerId: string }) {
    ...
    const max = tricks
    return (
      <div key={`${roundNum}-${playerId}`} className={`border-b grid grid-cols-1 grid-rows-2 transition-all duration-200 ${getPlayerCellBackgroundStyles(rState)}`}>
        {rState === 'locked' && (
          <>
        ...
        {rState === 'bidding' && (
          <>
            <div className="border-b flex items-center justify-between px-1 py-0.5">
              <Button size="sm" variant="outline" className="h-4 w-4 p-0 bg-white/80 hover:bg-white border-sky-300 text-sky-700" onClick={() => decrementBid(roundNum, playerId)} disabled={bid <= 0}><Minus className="h-2 w-2" /></Button>
              <span className="text-[0.7rem] font-bold min-w-[1rem] text-center text-sky-900 bg-white/60 px-1 rounded">{bid}</span>
              <Button size="sm" variant="outline" className="h-4 w-4 p-0 bg-white/80 hover:bg-white border-sky-300 text-sky-700" onClick={() => incrementBid(roundNum, playerId, max)} disabled={bid >= max}><Plus className="h-2 w-2" /></Button>
            </div>
        ...
        {rState === 'complete' && (
          <>
            ...
            <div className="flex items-center justify-center gap-1 py-0.5">
              <Button size="sm" variant={made === true ? 'default' : 'outline'} className="h-5 w-5 p-0 bg-white/80 hover:bg-white border-orange-300" onClick={() => toggleMade(roundNum, playerId, true)}><Check className="h-3 w-3" /></Button>
              <Button size="sm" variant={made === false ? 'destructive' : 'outline'} className="h-5 w-5 p-0 bg-white/80 hover:bg-white border-orange-300" onClick={() => toggleMade(roundNum, playerId, false)}><X className="h-3 w-3" /></Button>
            </div>
          </>
        )}
        {rState === 'scored' && (
          <>
            ...
            <div className="flex items-center justify-between px-1 py-0.5">
              ...
              <span className="font-bold text-[0.65rem] text-emerald-900">{(state.scores[playerId] ?? 0)}</span>
            </div>
          </>
        )}
      </div>
      ...
      <Card className="overflow-hidden shadow-lg">
        ...
        {Array.from({ length: 10 }, (_, i) => ({ round: i + 1, tricks: 10 - i })).map((round) => (
          ...
          >
            ...
            {players.map((p) => (
              <PlayerCell key={`${round.round}-${p.id}`} roundNum={round.round} tricks={round.tricks} playerId={p.id} />
            ))}
          </div>
        ))}
      </Card>
