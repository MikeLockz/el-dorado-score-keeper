...
export async function initInstance(dbName: string, channelName = `chan-${dbName}`, useChannel = true) {
...
export async function withTabs(k: number, opts?: { prefix?: string; useChannel?: boolean }) {
  ...
  const tabs = await Promise.all(Array.from({ length: k }, () => createInstance({ dbName, channelName, useChannel })))
  return {
    dbName,
    channelName,
    tabs,
    async close() { for (const t of tabs) t.close() },
  }
}
