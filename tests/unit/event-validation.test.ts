...
describe('event validation on append', () => {
  it('rejects invalid payload with warn', async () => {
    const warns: any[] = []
    const a = await createInstance({ dbName: `val-${Math.random()}`, onWarn: (c, i) => warns.push({ c, i }) })
    const bad: any = { type: 'player/added', payload: { id: '' /* empty id */, name: '' }, eventId: 'x', ts: Date.now() }
