...
const asyncCtx = <T extends Record<string, unknown>>(params: T) =>
  ({ params: Promise.resolve(params) }) as { params: Promise<T> };

describe('URL view metadata', () => {
  it('includes game id in single-player live metadata', async () => {
    const result = await spLiveMetadata(asyncCtx({ gameId: 'abc123' }));
  ...
  it('includes sub-route metadata for single-player scorecard and summary', async () => {
    const scorecard = await spScorecardMetadata(asyncCtx({ gameId: 'game-1' }));
    const summary = await spSummaryMetadata(asyncCtx({ gameId: 'game-1' }));
  ...
  it('produces scorecard metadata with session id', async () => {
    const live = await scorecardLiveMetadata(asyncCtx({ scorecardId: 'session-9' }));
    const summary = await scorecardSummaryMetadata(asyncCtx({ scorecardId: 'session-9' }));
  ...
  it('embeds entity ids for player and roster detail pages', async () => {
    const player = await playerDetailMetadata(asyncCtx({ playerId: 'player-42' }));
    const roster = await rosterDetailMetadata(asyncCtx({ rosterId: 'roster-99' }));
  ...
  it('produces archived game metadata', async () => {
    const meta = await gameDetailMetadata(asyncCtx({ gameId: 'archived-1' }));
