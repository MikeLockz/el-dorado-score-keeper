...
describe('IndexedDB schema migrations', () => {
  ...
  it('upgrades v1 -> v2 by adding games store and index', async () => {
    ...
    r1.onupgradeneeded = () => {
      ...
      const addReq = tx.objectStore('events').add({ type: 'init', eventId: 'seed-1', ts: Date.now() })
      addReq.onerror = () => {}
    }
    await new Promise<void>((res, rej) => { r1.onsuccess = () => res(); r1.onerror = () => rej(r1.error) })
    r1.result.close()
    ...
    const count = await new Promise<number>((res, rej) => { countReq.onsuccess = () => res(countReq.result); countReq.onerror = () => rej(countReq.error) })
    expect(count).toBe(1)
