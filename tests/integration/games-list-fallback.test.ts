...
async function seedGamesDbWithoutIndex(name: string) {
  ...
  await new Promise<void>((res, rej) => { open.onsuccess = () => res(); open.onerror = () => rej(open.error) })
  const db = open.result
  ...
  store.put({ id: 'a', title: 'A', createdAt: 100, finishedAt: 200, lastSeq: 1, summary: { players: 0, scores: {}, playersById: {}, winnerId: null, winnerName: null, winnerScore: null }, bundle: { latestSeq: 1, events: [] } })
  store.put({ id: 'b', title: 'B', createdAt: 300, finishedAt: 400, lastSeq: 1, summary: { players: 0, scores: {}, playersById: {}, winnerId: null, winnerName: null, winnerScore: null }, bundle: { latestSeq: 1, events: [] } })
