...
import { archiveCurrentGameAndReset, restoreGame, listGames, getGame, deleteGame, GAMES_DB_NAME, DEFAULT_DB_NAME } from '@/lib/state/io'
...
describe('archive and restore flows', () => {
  it('archives current game and seeds roster; restore replaces state', async () => {
    ...
    expect(Object.keys(scores).length === 0 || roster.every(id => (scores as any)[id] === 0)).toBe(true)
