...
Clicking add player and add roster should use already implemented modal UI for consistency and reusability. Look how Start a new game modal is implemented and follow similar patterns.

Change the detail button in single player to instead view the game summary view at any time during the game and remove any legacy views that are abandoned.

When in the summary view, clicking the details button again will close the summary view and return to the game.

Add more information to the round summary view. Include a full score card for all previous rounds that show what each player bid and what they actually scored. Include a total score for each player at the bottom.
