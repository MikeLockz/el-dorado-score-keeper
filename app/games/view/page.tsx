    ...
    <div className="p-3 max-w-xl mx-auto">
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <div className="text-sm text-slate-700 dark:text-slate-200">Finished {formatDateTime(game.finishedAt)}</div>
        </div>
      ...
      <Card className="p-2">
        <div className="text-sm text-slate-800 dark:text-slate-200">Events: {game.bundle.events.length} • Seq: {game.lastSeq}</div>
      </Card>
      ...
      <Card className="p-2 mt-3">
          ...
          <div>
            ...
            <div className="space-y-0.5">
              <div>
                ...
                <span className="font-medium">{stats?.leaders.mostTotalBid ? `${stats.leaders.mostTotalBid.name} (${stats.leaders.mostTotalBid.totalBid})` : '—'}</span>
              </div>
              <div>
                <span className="text-slate-800 dark:text-slate-200 mr-1">Least Total Bid:</span>
                <span className="font-medium">{stats?.leaders.leastTotalBid ? `${stats.leaders.leastTotalBid.name} (${stats.leaders.leastTotalBid.totalBid})` : '—'}</span>
              </div>
              <div>
                <span className="text-slate-800 dark:text-slate-200 mr-1">Highest Single Bid:</span>
                <span className="font-medium">{stats?.leaders.highestSingleBid ? `${stats.leaders.highestSingleBid.name} (R${stats.leaders.highestSingleBid.round}: ${stats.leaders.highestSingleBid.bid})` : '—'}</span>
              </div>
              <div>
                <span className="text-slate-800 dark:text-slate-200 mr-1">Biggest Single Loss:</span>
                <span className="font-medium">{stats?.leaders.biggestSingleLoss ? `${stats.leaders.biggestSingleLoss.name} (R${stats.leaders.biggestSingleLoss.round}: -${stats.leaders.biggestSingleLoss.loss})` : '—'}</span>
              </div>
            </div>
          </div>
          <div>
            ...
            <div className="space-y-0.5">
              <div><span className="text-slate-800 dark:text-slate-200 mr-1">Total Points Bid:</span><span className="font-medium">{stats?.totals.totalPointsBid ?? '—'}</span></div>
              <div><span className="text-slate-800 dark:text-slate-200 mr-1">Hands Won:</span><span className="font-medium">{stats?.totals.totalHandsWon ?? '—'}</span></div>
              <div><span className="text-slate-800 dark:text-slate-200 mr-1">Hands Lost:</span><span className="font-medium">{stats?.totals.totalHandsLost ?? '—'}</span></div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 text-sm mt-3">
          <div>
            ...
            <div className="text-slate-800 dark:text-slate-200">{stats ? formatDateTime(stats.timing.startedAt) : '—'}</div>
          </div>
          <div>
            <div className="font-bold">Ended</div>
            <div className="text-slate-800 dark:text-slate-200">{stats ? formatDateTime(stats.timing.finishedAt) : '—'}</div>
          </div>
          <div>
            <div className="font-bold">Duration</div>
            <div className="text-slate-800 dark:text-slate-200">{stats ? formatDuration(stats.timing.durationMs) : '—'}</div>
          </div>
        </div>
      </Card>
    </div>
